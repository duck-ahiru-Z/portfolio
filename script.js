document.addEventListener('DOMContentLoaded', () => {
  // 豆電球切り替え（クリックトグル）ギミック
  const lightbulbBtn = document.getElementById('lightbulb-btn');
  const lightbulbImg = document.getElementById('lightbulb-img');
  if (lightbulbBtn && lightbulbImg) {
    let isLit = false;
    lightbulbBtn.addEventListener('click', () => {
      isLit = !isLit;
      lightbulbImg.src = isLit ? 'img/light-on.png' : 'img/light-off.png';
      lightbulbImg.alt = isLit ? '点灯している電球' : '消灯している電球';
    });
  }

  // バグ撃退（クリック ➔ crash ➔ sleep ➔ 落下）ギミック
  function setupBugGimmick() {
    const widgetsContainer = document.querySelector('.hero-widgets');
    if (!widgetsContainer) return;

    // bug-btn が無ければ新しく作ってコンテナへ追加
    let bugBtn = document.getElementById('bug-btn');
    if (!bugBtn) {
      bugBtn = document.createElement('button');
      bugBtn.id = 'bug-btn';
      bugBtn.className = 'hero-bug-btn';
      bugBtn.setAttribute('aria-label', 'バグ撃退');
      bugBtn.innerHTML = `<img id="bug-img" src="img/bug-normal.png" alt="バグ" class="hero-bug-img">`;
      widgetsContainer.appendChild(bugBtn);
    }

    const bugImg = document.getElementById('bug-img');
    if (!bugImg) return;

    let isCrashed = false;
    bugBtn.addEventListener('click', () => {
      if (isCrashed) return; // 既にクラッシュ（落下中）なら操作不可
      isCrashed = true;

      // 1. 画像を bug-crash.png に切り替える (ヒットの瞬間)
      bugImg.src = 'img/bug-crash.png';

      // 2. 300ms後に sleep 画像に切り替えて、画面の下へ回転しながら落下
      setTimeout(() => {
        bugImg.src = 'img/bug-sleep.png';
        
        // 落下時に位置がジャンプしないよう、画面上の絶対位置を固定 (fixed)
        const rect = bugBtn.getBoundingClientRect();
        bugBtn.style.top = `${rect.top}px`;
        bugBtn.style.left = `${rect.left}px`;
        bugBtn.style.width = `${rect.width}px`;
        bugBtn.style.height = `${rect.height}px`;
        bugBtn.classList.add('bug-falling');

        // 3. アニメーション完了後 (2.2秒後) にDOMから削除 ＆ さらに4秒後に新しいバグを優しく復活 (リスポーン)
        setTimeout(() => {
          bugBtn.remove();
